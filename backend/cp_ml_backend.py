...
def run_scheduler(data):
    ...
    machine_names  = data['machine_types']
    ...
    for mt in machine_names:
        unit = mt["name"]  # Already has "[1]", "[2]" etc.
        machines.append(unit)
        for op in mt.get("operations", operations):
    ...
    alpha = 1
    beta = 1
    model.Minimize(alpha * makespan + beta * sum(late_vars))
